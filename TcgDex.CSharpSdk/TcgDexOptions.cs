...
public sealed class TcgDexOptions
{
    ...
    /// <remarks>
    /// <para>
    /// <b>This is how you point the SDK at a different server</b> — an unofficial
    /// mirror, a self-hosted instance, or a local test server. Set
    /// <see cref="GraphQlEndpoint"/> alongside it: the two are separate because
    /// GraphQL lives outside the language segment, so changing this one alone
    /// leaves GraphQL queries going to the official host.
    /// </para>
    /// <para>
    /// <b>The trailing slash is required</b>, and <see cref="Validate"/> rejects
    /// an address without one. Request paths resolve relative to this address, so
    /// without the slash the last segment is treated as a file name and replaced:
    /// <c>https://mine/v2</c> + <c>en/cards/x</c> resolves to
    /// <c>https://mine/en/cards/x</c>, silently losing the API root. The symptom
    /// is every lookup returning "not found", which points nowhere near the cause.
    /// </para>
    /// <para>
    /// A third-party server sees your requests and serves whatever it likes. The
    /// data is public and read-only so there is little to leak, but a bad server
    /// can return wrong card data. A non-HTTPS address is logged as a warning.
    /// </para>
    /// </remarks>
...
#pragma warning disable S3928
        ...
        // A missing trailing slash is not cosmetic. Request paths are resolved
        // RELATIVE to this address, so without the slash the last segment is
        // treated as a file name and REPLACED: 'https://mine/v2' + 'en/cards/x'
        // resolves to 'https://mine/en/cards/x', silently dropping '/v2'. The
        // server answers 404, the SDK maps 404 to null, and the caller is told a
        // card that exists does not.
        //
        // This guard previously existed only on the failover endpoint list. That
        // list was removed in 0.5.0, and BaseAddress — which had always had the
        // same hazard and never had the check — became the only way to reach a
        // custom server. Indexed rather than EndsWith: the char overload CA1865
        // asks for does not exist on netstandard2.0, and the string overload is
        // what it rejects. AbsolutePath of an absolute URI is never empty.
        string basePath = BaseAddress.AbsolutePath;

        if (basePath[basePath.Length - 1] != '/')
        {
            throw new ArgumentException(
                $"BaseAddress must end with '/', but was '{BaseAddress}'. " +
