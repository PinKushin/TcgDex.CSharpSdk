...
| **[Logging and tracing](observability.md)** | Structured logs and OpenTelemetry spans, free when unused. |
...
- **Observable** — structured `ILogger` output and `ActivitySource` spans,
  with no dependency on any telemetry vendor.
