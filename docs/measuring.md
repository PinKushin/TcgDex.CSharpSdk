...
| **Failover request count under concurrency** | Concurrent callers hitting a dead endpoint collapse to one discovery request. | **Deliberately not asserted, and the claim above is false.** `ConcurrentRequests_AllGetACorrectAnswer` covers what matters — 50 simultaneous callers all get a correct answer, with no torn read of the shared cooldown array — but it asserts no request count, because threads racing through the window before the endpoint is marked failed each spend one attempt on it. Pinning a number would be asserting the thread scheduler. The race is benign and bounded: every one of those requests was going to contact that endpoint anyway, so the worst case equals the traffic sent with no failover configured at all. |
