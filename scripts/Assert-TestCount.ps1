...
A FLOOR rather than an exact count, deliberately: an exact match makes every added test a red
build (this repo hand-bumped its old union total twice in one week for exactly that reason -
9499f9e, c879724), while a floor still catches a host dying mid-run or a filter matching nothing,
which are the failures that hide. Lower the floor only when tests were deliberately removed.
